<div align="center">

<!-- Animated Code Logo -->
<img src="https://user-images.githubusercontent.com/74038190/212284087-bbe7e430-757e-4901-90bf-4cd2ce3e1852.gif" width="280" alt="Code Animation"/>

<!-- Animated Header -->
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=28&duration=4000&pause=1000&color=58A6FF&center=true&vCenter=true&multiline=true&repeat=false&width=700&height=100&lines=Hey+%F0%9F%91%8B%2C+I'm+Ashutosh+Kumar;Software+Developer+%7C+CSE+%40+IIIT+Bhopal" alt="Typing SVG" />

<!-- Animated Tagline -->
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=18&duration=3000&pause=1000&color=8B949E&center=true&vCenter=true&width=700&height=35&lines=%F0%9F%8E%93+Indian+Institute+of+Information+Technology%2C+Bhopal;%F0%9F%92%BB+Building+Pillar+%E2%80%93+Local-First+Collaborative+Study+Platform;%E2%9C%85+750%2B+DSA+Problems+%7C+%F0%9F%9B%A1%EF%B8%8F+LeetCode+Knight" alt="Tagline" />

<br/>

<!-- Social Badges -->
<a href="https://linkedin.com/in/ashutosh-kumar879">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>
<a href="mailto:ashutoshkumariiitb@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
</a>

</div>

---

## 🧑‍💻 About Me

```json
{
    "name": "Ashutosh Kumar",
    "location": "Bhopal, India",
    "role": "Full Stack Development Intern @ QuickIntell (US Remote)",
    "education": {
        "degree": "B.Tech in Computer Science & Engineering",
        "university": "Indian Institute of Information Technology (IIIT), Bhopal",
        "timeline": "2023 - 2027",
        "cgpa": "8.58/10"
    },
    "currently_building": "Pillar – Local-First Collaborative Study Platform",
    "interests": ["Software Engineering", "System Design", "Competitive Programming", "Full Stack Development"]
}
```

---

## 🛠️ Tech Stack

<div align="center">

### Languages
<img src="https://skillicons.dev/icons?i=cpp,typescript,javascript,python" />

### Frameworks & Libraries
<img src="https://skillicons.dev/icons?i=nextjs,react,express,tailwind,redux,prisma" />

### Databases & Cloud
<img src="https://skillicons.dev/icons?i=postgres,mongodb,redis,docker" />

### Tools
<img src="https://skillicons.dev/icons?i=git,vscode,vercel,aws,postman" />

</div>

---

## 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=CodeCenturian&show_icons=true&theme=github_dark&hide_border=true&count_private=true&include_all_commits=true" height="180" alt="GitHub Stats"/>
<img src="https://github-readme-streak-stats.herokuapp.com/?user=CodeCenturian&theme=github_dark&hide_border=true" height="180" alt="GitHub Streak"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=CodeCenturian&layout=compact&theme=github_dark&hide_border=true&langs_count=8" alt="Top Languages"/>

<br/>

<!-- GitHub Trophies -->
<img src="https://github-profile-trophy.vercel.app/?username=CodeCenturian&theme=onestar&no-frame=true&row=1&column=7" alt="GitHub Trophies"/>

<br/>

<!-- Contribution Graph -->
<a href="https://github.com/CodeCenturian">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=CodeCenturian&theme=github-dark&area=true&hide_border=true" width="95%" alt="Contribution Graph"/>
</a>

<img src="https://raw.githubusercontent.com/platane/snk/output/github-contribution-grid-snake-dark.svg" width="100%"/>

</div>

---

## 🏆 Featured Projects

<div align="center">

| Project | Tech Stack | Highlights |
|:-------:|:----------:|:-----------|
| **Pillar** | React 19, cr-sqlite, WASM, Zustand | 🔄 Local-first sync with CRDTs • 🖼️ Canvas annotations • ⚡ Zero-latency rendering |
| **Baskit** | MERN, Redis, Stripe | 📈 500+ daily transactions • ⚡ 40% reduced DB load • 💳 Secure Stripe webhooks |
| **ThinkYr** | MERN, Socket.io, Redux Toolkit | ✍️ Autosave every 3s • 🛡️ Upstash rate limiting • 🔄 Real-time collaboration |

</div>

---

## 🏅 Achievements

<div align="center">

|  | Achievement | Details |
|:---:|:---|:---|
| 🛡️ | **LeetCode Knight** | Rating **1854** |
| ⭐ | **CodeChef 3 Star** | Max Rating **1634** |
| ✅ | **Problem Solver** | **750+** DSA problems solved |
| 🎓 | **Academic Excellence** | Top **5%** at IIIT Bhopal |
| 🎤 | **Branch Representative** | Faculty-student liaison |

</div>

---

<div align="center">

### 💬 Let's Connect!

<a href="https://linkedin.com/in/ashutosh-kumar879">
  <img src="https://img.shields.io/badge/Let's_connect_on_LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>
<a href="mailto:ashutoshkumariiitb@gmail.com">
  <img src="https://img.shields.io/badge/Send_me_an_email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
</a>

<br/><br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=2,3,4,5,6&height=100&section=footer&animation=twinkling"/>

</div>
